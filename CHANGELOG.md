...
#### 高精度数值处理系统
- **新增内容**: 
  - 实现数字类型到decimal类型的自动转换功能，提高数值计算精度
  - 在数据加载过程中自动检测并转换数字列为decimal类型
  - 支持自定义decimal精度设置，默认为28位精度
- **技术实现**:
  - 在`LightweightDataUtils`类中添加`to_decimal`静态方法，支持多种数值类型转换
  - 在`LightweightDataFrame`类中添加`convert_numeric_to_decimal`方法，自动检测数字列
  - 在`DataManager`类中集成decimal转换功能，确保所有数据加载流程都支持高精度转换
  - 完善的异常处理机制，转换失败时不影响程序正常运行
- **用户体验**: 所有数值计算现在都使用高精度decimal类型，避免浮点数精度丢失问题
- **影响范围**: 数据加载、数值计算和显示的所有环节

### 🔧 代码优化

#### 数据处理流程增强
- **优化内容**: 
  - 优化大数据集处理时的decimal转换性能
  - 在分块处理数据时同步应用decimal转换
  - 增强数值类型检测的准确性和效率
- **技术实现**:
  - 在`get_data_for_level`方法中集成decimal转换
  - 在`_process_large_dataset`方法中为每个数据块应用decimal转换
  - 添加`_convert_numeric_to_decimal`私有方法统一处理转换逻辑
- **性能提升**: 保持高精度的同时确保数据处理性能不受影响
- **影响范围**: 所有数据加载和处理流程
