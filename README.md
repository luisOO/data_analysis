...
- 📋 **数据复制功能** - 支持复制单元格、整行JSON数据、Markdown格式表格和字段值
...
├── view.py                # 视图模块，GUI界面实现
├── data_model.py          # 数据模型，配置和数据管理
├── config_manager_ui.py   # 配置管理界面
├── utils.py               # 通用工具模块（新增）
├── config.json            # 配置文件
├── config_template.json   # 配置模板
...
程序提供多种数据复制方式，方便用户在不同场景下使用：

#### 表格右键菜单
在数据表格中右键点击任意行，可选择以下复制选项：

- **复制行** - 复制当前行的纯文本数据，字段间用制表符分隔
- **复制为JSON** - 将当前行数据复制为JSON格式，便于程序间数据交换
- **复制为Markdown** - 将当前行数据复制为Markdown表格格式，包含表头和数据行，适用于文档编写、Wiki页面等

#### 字段值复制
- **双击单元格** - 快速复制单个单元格的值
- **右键字段** - 在基本信息区域右键点击字段值，可复制该字段的内容

#### 使用场景
- **文档编写**: 使用Markdown格式复制，直接粘贴到支持Markdown的编辑器
- **数据分析**: 使用JSON格式复制，便于在其他分析工具中使用
- **报告制作**: 使用纯文本格式复制，粘贴到Excel或其他表格工具
